...
export default function GirlsVotePage() {
  ...
  const fetchProfiles = useCallback(async () => {
    try {
      ...
      if (!profilesData || profilesData.length === 0) {
        console.log('No male profiles found');
        setError('No profiles available at the moment.');
        return;
      }

      console.log(`Found ${profilesData.length} male profiles`);
  ...
  useEffect(() => {
    const loadSavedState = async () => {
      ...
      const savedState = localStorage.getItem(`votingState_girls_${userId}`);
      if (savedState) {
  ...
  useEffect(() => {
    const saveCompleteState = async () => {
      ...
      localStorage.setItem(`votingState_girls_${sessionUserId}`, JSON.stringify(stateToSave));
    };
  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      ...
      if (userGender !== 'female') {
        console.log('User is not female, redirecting to boys voting page');
  ...
  // Save vote state
  useEffect(() => {
    const saveVoteState = async () => {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) return;

      const userId = session.user.id;
      localStorage.setItem(`voteState_girls_${userId}`, JSON.stringify(voteState));
    };
  ...
  useEffect(() => {
    const loadVoteState = async () => {
      ...
      const userId = session.user.id;
      const savedVoteState = localStorage.getItem(`voteState_girls_${userId}`);
      if (savedVoteState) {
        setVoteState(JSON.parse(savedVoteState));
      }
    };
